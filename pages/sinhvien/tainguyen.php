                ...
                try {
                    ...
                    if ($idTaiKhoan) {
                        $sql = "SELECT f.*, sv.Ten AS TenSinhVien, sv.Lop, sv.MSSV
                                ...
                                AND td.ID_Dot = (SELECT ID_Dot FROM sinhvien WHERE ID_TaiKhoan = :id_sv)
                                ORDER BY f.NgayNop DESC";
                        $stmt = $conn->prepare($sql);
                        $stmt->execute(['id_sv' => $idTaiKhoan]);
                    } else {
                        $sql = "SELECT f.*, sv.Ten AS TenSinhVien, sv.Lop, sv.MSSV
                                FROM file f
                    ...
                    if (count($resources) > 0) {
                        ...
                        foreach ($resources as $resource) {
                            ...
                            echo '<div>';
                            echo '<div class="resource-title">' . htmlspecialchars($resource['TenHienThi'] ?? $resource['TenFile']) . '</div>';
