...
<body>
        ...
        function renderCalendar() {
            ...
            for (let i = 0; i < totalCells; i++) {
                ...
                // Ô trong tháng này
                    ...
                    cell.innerHTML = `<span class="date-number">${dayNum}</span>`;
                    ...
                    cell.addEventListener('click', function() {
                        // Khi click vào ngày, submit form để reload trang với ngày mới
                        // Sửa: lấy ngày/tháng/năm từ cellDate và format lại thành yyyy-mm-dd
    ...
    document.getElementById('btn-edit-task').addEventListener('click', function() {
        if (window.selectedTaskId) {
            // Lấy dữ liệu task đang chọn
            const name = document.getElementById('progress-task-name').textContent;
            const desc = document.getElementById('progress-task-description').textContent;
            ...
            // Set ngày cho input ẩn (giữ nguyên ngày đang xem)
            const yyyy = selectedDate.getFullYear();
            const mm = String(selectedDate.getMonth() + 1).padStart(2, '0');
            const dd = String(selectedDate.getDate()).padStart(2, '0');
            document.getElementById('task-date-input').value = `${yyyy}-${mm}-${dd}`;
