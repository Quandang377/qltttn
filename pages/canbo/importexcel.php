...
$stmt = $conn->prepare("SELECT ID, TenDot, Nam, BacDaoTao FROM dotthuctap WHERE TrangThai = 1");
$stmt->execute();
$dotthuctap = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Xử lý chọn đợt
$dot_duoc_chon = $_POST['dot_id'] ?? $_POST['dot_id_hidden'] ?? $id;
                            ...
                            <form method="post" id="form-chon-dot">
                                <table class="table table-striped table-bordered" id="table-dot">
                                    ...
                                    <?php if ($dot_duoc_chon): ?>
                                        <script>
                                            document.addEventListener('DOMContentLoaded', () => {
                                                const radio = document.querySelector('input[name="dot_id"][value="<?= $dot_duoc_chon ?>"]');
                                    ...
                                    <tbody>
                                        <?php foreach ($dotthuctap as $i => $dot): ?>
                                            <tr>
                                                <td>
                                                    <input type="radio" name="dot_id" value="<?php echo $dot['ID']; ?>"
                                                        <?php if ($dot_duoc_chon == $dot['ID'])
                                                            echo 'checked'; ?>>
                                                </td>
                                                <td><?php echo $i + 1; ?></td>
                                                <td><?php echo htmlspecialchars($dot['TenDot']); ?></td>
                                                <td><?php echo htmlspecialchars($dot['Nam']); ?></td>
                                                <td><?php echo htmlspecialchars($dot['BacDaoTao']); ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
                                </table>
                                <div class="col-md-8 col-md-offset-2 text-center">
                                    <button type="submit" class="btn btn-primary" style="margin-top: 10px;"
                                        name="chon_dot" id="btn-dot">Chọn đợt</button>
                                </div>
                            </form>
                        </div>
                    </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <input type="file" name="excel_file" class="form-control" required
                                        accept=".xls,.xlsx">
                                    <input type="hidden" name="dot_id_hidden"
                                        value="<?php echo htmlspecialchars($dot_duoc_chon); ?>">
                                </div>
